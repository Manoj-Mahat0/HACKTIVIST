...
class _UnifiedNavigationPageState extends State<UnifiedNavigationPage> {
  ...
  void _setupLocationDetection() {
    ...
    _locationDetectionService.onStatusChanged = (status) {
      if (mounted) {
        ...
        if (status == DetectionStatus.highConfidence) {
          _showSuccess('Location detected with high confidence');
        } else if (status == DetectionStatus.lowConfidence) {
          _showWarning('Location detection has low confidence. Consider scanning a QR code.');
        }
      }
    };
    ...
    _locationDetectionService.onDetectionError = (error) {
      if (mounted) {
        _showError('Location detection error: $error');
      }
    };
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (_nodes.isEmpty) {
          ...
          // Start automatic location detection after nodes are loaded
  ...
  Future<void> _startAutomaticLocationDetection() async {
    ...
    if (success) {
      ...
      _showSuccess('Automatic location detection started');
    } else {
      setState(() => _isDetectingLocation = false);
      _showWarning('Could not start location detection. Please select start location manually or scan a QR code.');
    }
  }
